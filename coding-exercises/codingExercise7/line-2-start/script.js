...
function gotData(incomingData){
  ...
  ;
    //a>b?1:0; -->> make judgement on whether a is larger than b, if yes, return 1; if no, return 0.
  function usaViz(){
    // let usaData = [incomingData[0]];
    // console.log(usaData);

    let elementsForPage = graphGroup.selectAll(".line").data([incomingData[0]])
    console.log(elementsForPage);

    elementsForPage.enter()
      .append("path")
      .attr("d", lineMaker)
      .attr("fill", "none")
      .transition()
  ...
  function chinaViz(){
    // let chinaData = [incomingData[1]];
    // console.log(chinaData);

    let elementsForPage = graphGroup.selectAll(".line").data([incomingData[1]])
    console.log(elementsForPage);
